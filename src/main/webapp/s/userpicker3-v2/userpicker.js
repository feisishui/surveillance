...
'<div id="' + conf.modalId + '" class="modal fade" style="z-index: 9996 !important;">'
+'  <div class="modal-dialog" style="height: 70% !important;">'
+'    <div class="modal-content" style="height: 100% !important;">'
+'      <div class="modal-header" style="height: 70px !important;">'
+'        <button type="button" class="close" data-dismiss="modal" aria-hidden="true" style="width: 14px; height: 14px;"></button>'
+'        <h3>选择用户</h3>'
+'      </div>'
+'      <div class="modal-body" style="height: 70% !important;overflow-y: auto;">'
+'        <div class="row">'
+'          <div class="col-md-4">'
...
+'          </div>'
+'  <table id="' + conf.modalId + '_grid" class="table table-hover" style="margin-bottom: 0px !important;">'
+'    <tbody id="' + conf.modalId + '_title">'
+'      <tr>'
//+'        <th width="10" class="m-table-check">&nbsp;</th>'
+'        <td><input id="selectAll" type="checkbox" class="selectedItem"></td>'
...
+'</div>');
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    html +=
                      '<tr>'
                        +'<td><input id="' + conf.modalId + '_item_' + i + '" type="' + (conf.multiple ? 'checkbox' : 'radio')
						+ '" class="selectedItem" name="name" value="'
                        + item.id + '" title="' + item.displayName + '"'
                        + checked + '></td>'
                        +'<td><label for="' + conf.modalId + '_item_' + i + '">' + item.displayName + '</label></td>'
                      +'</tr>'
                }
    ...
    $(document).delegate('#' + conf.modalId + '_title .selectedItem', 'click', function(e) {
        var isSelect = $(this).prop('checked');
        $('#' + conf.modalId + '_body .selectedItem').each(function(index){
            if(isSelect!=$(this).prop('checked'))
            {
                $(this).trigger("click")
            }
        })
    ...
    $(document).delegate('#' + conf.modalId + '_body .selectedItem', 'click', function(e) {
		if (conf.multiple) {
			var el = $(this);
			if (el.prop('checked')) {
				var html = '<span class="label label-default" font-size="100%" id="' + $(this).val() + '" title="' + $(this).attr('title') + '">' + $(this).attr('title') + '</span>';
