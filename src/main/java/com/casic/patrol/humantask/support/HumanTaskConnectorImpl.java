...
import com.casic.patrol.api.humantask.*;
...
import com.casic.patrol.humantask.listener.HumanTaskListener;
import com.casic.patrol.humantask.persistence.domain.TaskConfUser;
...
import com.casic.patrol.humantask.persistence.manager.TaskConfUserManager;
import com.casic.patrol.humantask.persistence.manager.TaskDeadlineManager;
...
import com.casic.patrol.spi.process.ProcessTaskDefinition;
...
public class HumanTaskConnectorImpl implements HumanTaskConnector {
    ...
    public FormDTO findTaskForm(String humanTaskId) {
        ...
        if (humanTaskDto.getTaskId() != null) {
            ...
            // .getTaskId());
            com.casic.patrol.spi.humantask.FormDTO taskFormDto = taskDefinitionConnector
    ...
    public void completeTask(String humanTaskId, String userId, String action,
        ...
        TaskInfo taskInfo2 = taskInfoManager.get(Long.parseLong(humanTaskId));
        if (taskInfo2 != null) {
            Set<TaskParticipant> set = taskInfo2.getTaskParticipants();
            if ((set != null && set.size() > 0) && "complete".equals(taskInfo2.getStatus())) {
    ...
    public Page findPersonalTasks(String userId, String tenantId, int pageNo,
        ...
        Page page = taskInfoManager
                .pagedQuery(
                        "select t from TaskInfo t left join t.taskParticipants p where (t.assignee=? or p.ref=?) and t.tenantId=? and t.status='active'", pageNo, pageSize, userId, userId, tenantId);
     ...
     * 待办任务(按照任务分类).
     */
    public Page findPersonalTasksByName(String userId, String tenantId, int pageNo,
                                  int pageSize,String taskName) {
        Page page = null;
        if (taskName != null && taskName.length() > 0) {
            page = taskInfoManager
                    .pagedQuery(
                            "select t from TaskInfo t left join t.taskParticipants p where " +
                                    "(t.assignee=? or p.ref=?) and t.tenantId=? and t.name=? and " +
                                    "t.status='active'", pageNo, pageSize, userId,
                            userId, tenantId,taskName);
        } else {
            page = taskInfoManager
                    .pagedQuery(
                            "select t from TaskInfo t left join t.taskParticipants p where " +
                                    "(t.assignee=? or p.ref=?) and t.tenantId=? and " +
                                    "t.status='active'", pageNo, pageSize,
                            userId, userId, tenantId);
        }
        List<TaskInfo> taskInfos = (List<TaskInfo>) page.getResult();
        List<ToDoTaskDTO> toDoTaskDTOs = this.convertToDoTaskDtos(taskInfos);
        page.setResult(toDoTaskDTOs);

        return page;
    }

    public List<HumanTaskDTO> findPersonalTasks(String userId, String tenantId, String lastId) {
        List<TaskInfo> taskInfos = taskInfoManager.pageQueryByLastId(userId,tenantId,lastId);
        List<HumanTaskDTO> humanTaskDtos = this.convertHumanTaskDtos(taskInfos);
        return humanTaskDtos;
    }

    public String getTaskNumber(String userId, String tenantId) {
        int number = taskInfoManager.getTaskNumber(userId, tenantId);
        return String.valueOf(number);
    }

    public List<HumanTaskDTO> findFinishedPersonalTasks(String userId, String tenantId, String lastId) {
        List<TaskInfo> taskInfos = taskInfoManager.findFinishedTaskByLastId(userId,tenantId,lastId);
        List<HumanTaskDTO> humanTaskDtos = this.convertHumanTaskDtos(taskInfos);
        return humanTaskDtos;
    }
    /**
    ...
    public Page findFinishedTasks(String userId, String tenantId, int pageNo,
        ...
        Page page = taskInfoManager
                .pagedQuery(
                        "from TaskInfo where assignee=? and tenantId=? and status='complete' order by completeTime desc ",
    ...
    public HumanTaskDTO convertHumanTaskDto(TaskInfo taskInfo) {
        ...
        TaskOvertimeInfo info = TaskOvertimeInfoManager.getMostSeriousInfo(
                taskOvertimeInfoManager.getUnfinishedBy(
                        taskInfo.getProcessInstanceId(),
                        taskInfo.getProcessBusinessKey()));
        if (info != null) {
            humanTaskDto.setOvertimeLevel(OvertimeLevel.valueOf(info.getLevelCode()));
        } else {
            humanTaskDto.setOvertimeLevel(OvertimeLevel.UNKNOWN);
        }
    ...
    public List<ToDoTaskDTO> convertToDoTaskDtos(
            Collection<TaskInfo> taskInfos) {
    ...
    public ToDoTaskDTO convertToDoTaskDto(TaskInfo taskInfo) {
        if (taskInfo == null) {
            return null;
        }

        ToDoTaskDTO toDoTaskDTO = new ToDoTaskDTO();
        beanMapper.copy(taskInfo, toDoTaskDTO);
        ...
        if (taskInfo.getTaskInfo() != null) {
            toDoTaskDTO.setParentId(Long.toString(taskInfo.getTaskInfo()
                    .getId()));
        }

        if (!taskInfo.getTaskInfos().isEmpty()) {
            List<ToDoTaskDTO> children = this.convertToDoTaskDtos(taskInfo
                    .getTaskInfos());
            toDoTaskDTO.setChildren(children);
        }
        if (taskInfo.getProcessInstanceId() != null) {
            ...
            if (record!=null) {
                ...
                if (props.containsKey("position")) {
                    ...
                    String position = props.get("position").getValue();
                    if (position != null && position.split(",").length > 2) {
                        sb.append(position.split(",")[2]);
                    }
                    if (props.containsKey("positionDesp") &&
                            null != props.get("positionDesp").getValue()) {
                        sb.append(props.get("positionDesp").getValue());
                    } else {
                        sb.append("无");
                    }
                    toDoTaskDTO.setEventPosition(sb.toString());
                }
