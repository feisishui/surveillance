package com.casic.patrol.bpm.graph;
...
public class ActivitiHistoryGraphBuilder {
    ...
    public static final Set<String> checkDB = new HashSet<String>(Arrays.asList(
            "oracle"
    ));
    ...
    public void fetchHistoricActivityInstances() {
        ...
        if (checkDB.contains(Context.getProcessEngineConfiguration().getDatabaseType())) {
            if (historicActivityInstances != null && historicActivityInstances.size() > 1) {
                HistoricActivityInstance[] result = historicActivityInstances.toArray(
                        new HistoricActivityInstance[historicActivityInstances.size()]
                );
                Arrays.sort(result, new Comparator<HistoricActivityInstance>() {
                    @Override
                    public int compare(HistoricActivityInstance o1, HistoricActivityInstance o2) {
                        return new Long(Long.parseLong(o1.getId()) - Long.parseLong(o2.getId())).intValue();
                    }
                });
                historicActivityInstances = Arrays.asList(result);
            }
        }
    }
