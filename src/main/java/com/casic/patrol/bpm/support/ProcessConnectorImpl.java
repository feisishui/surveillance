...
import com.casic.patrol.bpm.cmd.FindFirstTaskFormCmd;
import com.casic.patrol.bpm.persistence.domain.BpmConfForm;
import com.casic.patrol.bpm.persistence.domain.BpmProcess;
import com.casic.patrol.bpm.persistence.manager.BpmConfFormManager;
...
import com.casic.patrol.spi.humantask.TaskUserDTO;
import com.casic.patrol.spi.process.FirstTaskForm;
...
public class ProcessConnectorImpl implements ProcessConnector {
    ...
    public FormDTO findStartForm(String processDefinitionId) {
        ...
        if ((!firstTaskForm.isExists())
            ...
            // 再从数据库里找一遍配置
            com.casic.patrol.spi.humantask.FormDTO humantaskFormDto = taskDefinitionConnector
    ...
    public Page findRunningProcessInstances( String tenantId,
                                            Page page) {
        HistoryService historyService = processEngine.getHistoryService();
    ...
    /**
     * 获取所有running process instances,然后再通过参数排序
     * @param tenantId
     * @return
     */
    public List<HistoricProcessInstance> findAllRunningProcessInstances(
            String tenantId) {
        HistoryService historyService = processEngine.getHistoryService();
        HistoricProcessInstanceQuery query = historyService
                .createHistoricProcessInstanceQuery()
     ...
     * 流程实例.
     */
    public Page findProcessInstances(String tenantId) {
        Page page = new Page();
        RuntimeService runtimeService = processEngine.getRuntimeService();
        long count = runtimeService.createProcessInstanceQuery()
                .processInstanceTenantId(tenantId).count();
